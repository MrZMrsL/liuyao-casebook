...
const SITE = {
  ...
  updated: '2026-08-15',
  stats: {
    total: 7, pending: 2, right: 4.5, wrong: 1.5, acc: 75,
    categories: [
      { name: '射覆', count: 2, right: 1, wrong: 1, acc: '50%' },
      { name: '失物', count: 2, right: 1.5, wrong: 0.5, acc: '75%' },
      { name: '财运', count: 1, right: 1, wrong: 0, acc: '100%' },
      { name: '行人出行', count: 1, right: 1, wrong: 0, acc: '100%' },
      { name: '其他', count: 2, right: 0, wrong: 0, acc: '待验证' },
      { name: '婚姻/疾病/官非/天气', count: 0, right: 0, wrong: 0, acc: '—' },
    ]
  },
  calibration: [
    ...
    { id: 'Case-006', cat: '其他', conf: 70, result: '待验证', note: '复合问事（天数/地点/去留），天数取象较集中，方位与去留细节置信稍降' },
    { id: 'Case-007', cat: '行人出行', conf: 75, result: '对', note: '三重否定信号叠加，结论果断正确；父母空亡动精准对应"课表排错"；官鬼伏藏值日精准对应"校方今日有变动"' },
  ],
};
...
const RULES = [
  ...
  { id: 'R18', status: '生效', cat: '行人出行', text: '行人占·父母空亡动 = 文书/课表/通知安排之变，主不来：父母为文书课表之象，空动则文书不实（★★★★★）', from: 'Case-007' },
  { id: 'R19', status: '生效', cat: '行人出行', text: '行人占·世爻空亡 = 约定根基不实：非对方单方面问题，而是约定基础动摇，世空之日事难落实（★★★★★）', from: 'Case-007' },
  { id: 'R20', status: '生效', cat: '行人出行', text: '行人占·子孙休囚+变卦六冲 = 冲散不来：六冲在用神有气时主速来，无气时主冲散（★★★★☆）', from: 'Case-007' },
  { id: 'R21', status: '生效', cat: '行人出行', text: '官鬼伏藏值日辰 = 校方/领导/权威方今日有动作或变动，直接影响所问之事（★★★★☆）', from: 'Case-007' },
];
...
const GLOSSARY = {
  ...
  '纠偏规则': '从断错案例复盘中提炼的"下次必先检查"条款，编号 R1–R21，断卦前逐条对照。',
};
...
const CASES = [
  ...
  {
    id: 'Case-007', cat: '行人出行', date: '2026-08-15', result: '对', score: 1,
    title: '学生来不来',
    question: '学生还来不来？为什么不来？来的话为什么迟到？',
    time: '丙午年 丙申月 辛酉日 壬辰时',
    kong: '子丑', shensha: '驿马亥 · 桃花午 · 禄神酉',
    benName: '风天小畜', benGong: '巽宫 · 二世',
    bianName: '坎为水', bianGong: '坎宫 · 六冲',
    lines: [
      { god: '腾蛇', rel: '兄弟', gz: '辛卯', el: '木', yang: true, move: 'o', bian: { rel: '父母', gz: '戊子', el: '水', yang: false } },
      { god: '勾陈', rel: '子孙', gz: '辛巳', el: '火', yang: true, bian: { rel: '妻财', gz: '戊戌', el: '土', yang: true } },
      { god: '朱雀', rel: '妻财', gz: '辛未', el: '土', yang: false, ying: true, bian: { rel: '官鬼', gz: '戊申', el: '金', yang: false } },
      { god: '青龙', rel: '妻财', gz: '甲辰', el: '土', yang: true, move: 'o', fu: '官鬼 辛酉', bian: { rel: '子孙', gz: '戊午', el: '火', yang: false } },
      { god: '玄武', rel: '兄弟', gz: '甲寅', el: '木', yang: true, bian: { rel: '妻财', gz: '戊辰', el: '土', yang: true } },
      { god: '白虎', rel: '父母', gz: '甲子', el: '水', yang: true, move: 'o', shi: true, kong: true, bian: { rel: '兄弟', gz: '戊寅', el: '木', yang: false } },
    ],
    duanyu: [
      { t: '不来——子孙休囚（申月囚酉日泄）+ 世爻空亡 + 六冲变坎，三重否定叠加', ok: true },
      { t: '原因：课表/文书安排之变（父母空亡动）——实为班主任排错课表', ok: true },
      { t: '校方/权威方今日有动作（官鬼伏藏值日辰酉金透出）', ok: true },
      { t: '下午之约仍不来——世空未解，下午时辰进一步化泄子孙火气', ok: true },
    ],
    resultText: '早上课因班主任排错课表学生不来；改约下午，学生仍不愿来，改为次日。✅ 全中',
    review: '父母空亡动=课表文书之变（行人类首验）；世空=约定根基不实；官鬼伏值日辰=校方当日有动作；三重否定信号同向则结论果断。产出 R18–R21。',
    newRules: ['R18', 'R19', 'R20', 'R21'],
  },
  {
    id: 'Case-006', cat: '其他', date: '2026-08-14', result: '待验证', score: null,
    title: '年会出行三问',
    question: '年会去哪开？开几天？能不能不去？（复合问事）',
    time: '丙午年 丙申月 庚申日 壬午时',
    kong: '子丑', shensha: '驿马寅 · 桃花酉 · 禄神申',
    benName: '地泽临', benGong: '坤宫 · 三世',
    bianName: '风泽中孚', bianGong: '艮宫 · 游魂',
    lines: [
      { god: '腾蛇', rel: '子孙', gz: '癸酉', el: '金', yang: false, move: 'o', bian: { rel: '官鬼', gz: '辛卯', el: '木', yang: true } },
      { god: '勾陈', rel: '妻财', gz: '癸亥', el: '水', yang: false, move: 'o', ying: true, bian: { rel: '父母', gz: '辛巳', el: '火', yang: true } },
      { god: '朱雀', rel: '兄弟', gz: '癸丑', el: '土', yang: false, bian: { rel: '兄弟', gz: '辛未', el: '土', yang: false } },
      { god: '青龙', rel: '兄弟', gz: '丁丑', el: '土', yang: false, shi: true, kong: true, bian: { rel: '兄弟', gz: '丁丑', el: '土', yang: false } },
      { god: '玄武', rel: '官鬼', gz: '丁卯', el: '木', yang: true, bian: { rel: '官鬼', gz: '丁卯', el: '木', yang: true } },
      { god: '白虎', rel: '父母', gz: '丁巳', el: '火', yang: true, bian: { rel: '父母', gz: '丁巳', el: '火', yang: true } },
    ],
    duanyu: [
      { t: '天数 2 天（动爻数2 + 下卦兑数2 + 世应相隔二爻）', ok: 'pending' },
      { t: '地点东南近水之地，地名或带泽/水/风/南（父母巳火东南+变卦巽东南兑泽）', ok: 'pending' },
      { t: '可争取不去，但须在丑日/未日（出空/冲空）前后提出；否则大势趋向"去"', ok: 'pending' },
    ],
    resultText: '⏳ 待验证',
    review: '复合问事多象交汇：父母=年会组织、官鬼=工作约束、世爻=去留意愿、应爻=举办地。天数取象集中（三重皆指向2），方位与去留细节置信稍降（70%）。注：起卦 APP 世应标注与经典规则不符，断语以经典排盘为准。',
    newRules: [],
  },
];
...
const YICI = {
  ...
  '地泽临': { t: '临，元亨利贞。至于八月有凶。', y: ['初九：咸临，贞吉。', '九二：咸临，吉无不利。', '六三：甘临，无攸利。既忧之，无咎。', '六四：至临，无咎。', '六五：知临，大君之宜，吉。', '上六：敦临，吉无咎。'],
    v: { t: '以大临小、以上临下，大亨通而宜守正；然盛极转衰，须防后势。', y: ['以感化之心临事，守正吉祥。', '感化而临且行中道，吉无不利。', '以甜言蜜语临下，无所利；知忧而改，无咎。', '亲临其地、亲自处理，无咎。', '以智慧临事，是明君之宜，吉祥。', '敦厚临下，吉而无咎。'] } },
  '风泽中孚': { t: '中孚，豚鱼吉，利涉大川，利贞。', y: ['初九：虞吉，有它不燕。', '九二：鸣鹤在阴，其子和之。我有好爵，吾与尔靡之。', '六三：得敌，或鼓或罢，或泣或歌。', '六四：月几望，马匹亡，无咎。', '九五：有孚挛如，无咎。', '上九：翰音登于天，贞凶。'],
    v: { t: '内心诚信，能感豚鱼之微，吉祥；利涉险难，利守正。', y: ['安守诚信则吉，另有他求则不安。', '鹤鸣于树荫，小鹤相和——诚信相感，如美酒共享。', '遇到对手，或击鼓或罢兵，或哭或歌——心随外物起伏不定。', '月将圆时失去马匹——有所舍方有所随，无咎。', '诚信系结人心，无咎。', '鸡鸣之声虚闻于天——虚声无实，守此则凶。'] } },
  '地雷复': { t: '复，亨。出入无疾，朋来无咎。反复其道，七日来复，利有攸往。', y: ['初九：不远复，无祗悔，元吉。', '六二：休复，吉。', '六三：频复，厉无咎。', '六四：中行独复。', '六五：敦复，无悔。', '上六：迷复，凶，有灾眚。用行师，终有大败，以其国君，凶；至于十年，不克征。'],
    v: { t: '一阳来复，亨通；往来无疾，朋友来无咎；事物循环，七日而归，利有攸往。', y: ['走得不远即知返，无悔，大吉。', '安然回复，吉祥。', '屡失屡返，虽有危厉而无咎。', '行于众人之中而能独自复归正道。', '敦厚回复，无悔。', '迷途不返，凶而有灾；用以兴兵终至大败，累及主事者，十年不能再振。'] } },
  '山雷颐': { t: '颐，贞吉。观颐，自求口实。', y: ['初九：舍尔灵龟，观我朵颐，凶。', '六二：颠颐，拂经，于丘颐，征凶。', '六三：拂颐，贞凶，十年勿用，无攸利。', '六四：颠颐吉，虎视眈眈，其欲逐逐，无咎。', '六五：拂经，居贞吉，不可涉大川。', '上九：由颐，厉吉，利涉大川。'],
    v: { t: '养生之道，守正吉祥；察人颐养，贵在自食其力。', y: ['舍自己灵龟般的自养之能，却艳羡他人之食，凶。', '颠倒求养、违背常理，向高处乞食，前行凶险。', '违背颐养正道，守此则凶，十年不可为用，无所利。', '在上者下求于贤而吉，如虎视之专注、所欲相继，无咎。', '虽违常道，安居守正则吉，不可涉险。', '天下由其颐养，有危厉而吉，利涉大川。'] } },
  '风天小畜': { t: '小畜，亨。密云不雨，自我西郊。', y: ['初九：复自道，何其咎，吉。', '九二：牵复，吉。', '九三：舆说辐，夫妻反目。', '六四：有孚，血去惕出，无咎。', '九五：有孚挛如，富以其邻。', '上九：既雨既处，尚德载，妇贞厉。月几望，君子征凶。'],
    v: { t: '小有蓄聚，亨通；浓云自西郊来而雨未下——蓄而未发，宜待时。', y: ['回归自家正道，有何咎，吉祥。', '被牵引而同归，吉祥。', '车轮脱辐，夫妻反目——小有阻滞则生隙。', '心存诚信，忧惧退去，无咎。', '诚信系结，与邻共富。', '雨已下又已止，德积而满；阴柔守此则危；月将圆，此时进取则凶——盛满当戒。'] } },
  '坎为水': { t: '习坎，有孚，维心亨，行有尚。', y: ['初六：习坎，入于坎窞，凶。', '九二：坎有险，求小得。', '六三：来之坎坎，险且枕，入于坎窞，勿用。', '六四：樽酒簋贰，用缶，纳约自牖，终无咎。', '九五：坎不盈，祗既平，无咎。', '上六：系用徽纆，寘于丛棘，三岁不得，凶。'],
    v: { t: '重重险陷之中，心怀诚信、内心通达，前行自有崇尚。', y: ['险中又陷，落入深坑，凶。', '险中有险，所求仅得小成。', '来去皆险、险而且深，落坑之中切勿妄动。', '一樽酒、两簋食，以瓦缶相盛，自窗递入简约之约——险中以诚相交，终无咎。', '坎陷未满而将平，无咎。', '被绳索捆绑、置于荆棘，三年不得脱身，凶。'] } },
  '火泽睽': { t: '睽，小事吉。', y: ['初九：悔亡，丧马勿逐，自复；见恶人，无咎。', '九二：遇主于巷，无咎。', '六三：见舆曳，其牛掣，其人天且劓，无初有终。', '九四：睽孤，遇元夫，交孚，厉无咎。', '六五：悔亡，厥宗噬肤，往何咎。', '上九：睽孤，见豕负涂，载鬼一车，先张之弧，后说之弧，匪寇婚媾，往遇雨则吉。'],
    v: { t: '乖违背离之时，大事难成，小事吉祥。', y: ['悔恨消失；失马不追自会回来；见了厌恶之人也不必回避，无咎。', '于小巷中遇见主人，无咎。', '车被拖、牛被拽、人受刑——开局虽糟，终有结局。', '孤离之际遇到大丈夫，以诚信相交，有危厉而无咎。', '悔恨消失，同宗亲厚之人前来（如噬肤之亲），前往何咎。', '孤离多疑：见猪背泥、见车载鬼，先张弓后释弓——不是敌寇而是亲家；前往遇雨（阴阳和合）则吉。'] } },
};
